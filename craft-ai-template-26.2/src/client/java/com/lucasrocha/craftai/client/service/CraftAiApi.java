...
public class CraftAiApi {
    ...
    public CompletableFuture<String> askQuestion(
        ...
        ).thenApply(response -> parseAnswer(response))
                .exceptionally(error -> {
        ...
        try {
            ...
            if (error != null && error.has("code") && error.has("message")) {
                return new CraftAiApiException(
                        ...
                        playerMessageFor(error.get("code").getAsString(), error.get("message").getAsString())
                );
            }
    ...
    private static String playerMessageFor(String code, String backendMessage) {
        return switch (code) {
            ...
            case "AI_UNAVAILABLE" -> "The AI service is unavailable right now. Please try again shortly.";
            case "AI_INVALID_RESPONSE" -> "The AI service returned an unreadable answer. Please try again.";
            case "INVALID_REQUEST" -> "The backend rejected the game context. Check the log for details.";
